...
const useImages = (): { images: string[] } => {
  ...
  useEffect(() => {
    ...
    ): string[] => images.map((imageName) => namespaceFunction(imageName));

    const landscapeImages = generateImageNames(peopleImages, peopleNamespace);
    const itemImages = generateImageNames(kidsImages, kidsNamespace);

    const images = landscapeImages.concat(itemImages);
